...
Reference run, already verified and reproducible on camera if the live one is slow:
`blind-1823` — a recruitment agency drawn blind, four steps proven, the submit refused at the
gate, zero applications in the server's record. Saved output: `mission/HERO_VERIFICATION.txt`.
